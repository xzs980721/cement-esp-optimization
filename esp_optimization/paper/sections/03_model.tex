...
\label{eq:dindex}
\end{equation}
式中 $\sum_iw_{i,t}=1$，$Q_0,U_{i,0}$ 为数据中位参考点。$Q_0/Q_t$ 对应停留时间；$U_i^2$ 对应迁移驱动力及电场能耗的低阶近似。分场先验均值取
\begin{equation}
\bm w_0=(0.32,0.28,0.23,0.17),
\label{eq:weights}
\end{equation}
...
高温会改变烟气密度、离子迁移及粉尘比电阻，实验研究表明温度影响具有明显非线性\cite{xu2015hightemp}。本文采用以 $125^\circ\mathrm C$ 为中心的钟形修正
\begin{equation}
g_\Theta(\Theta)=\exp\left[-\kappa_\Theta
\left(\frac{\Theta-125}{s_\Theta}\right)^2\right],
\qquad s_\Theta=40^\circ\mathrm C.
\label{eq:tempfactor}
\end{equation}
它不将 125$^\circ$C 解释为普适最优温度，而是用于描述本工况区间内偏离参考温区后的有效迁移能力折减。
...
电场电耗在工作区间内与电压平方近似相关；振打机构的平均功耗与动作频率 $1/T_i$ 相关，多电场节能控制研究也支持按场分配能量的思路\cite{li2013ga}。建立
\begin{equation}
...
\label{eq:penalty}
\end{equation}
最终策略用另一随机种子生成的 10,000 个独立情景检查 $q_{0.95}(C)\le L$。算法固定基础种子为 2026，以确保可复现。

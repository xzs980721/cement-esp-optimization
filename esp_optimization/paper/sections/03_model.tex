...
\label{eq:dindex}
\end{equation}
式中 $\sum_iw_{i,t}=1$，$Q_0,U_{i,0}$ 为数据中位参考点。$Q_0/Q_t$ 对应停留时间；$U_i^2$ 对应迁移驱动力的低阶近似；$g_\Theta(\Theta_t)$对应温度修正；$g_M(\bm T_t,L_t)$对应积灰修正。附件没有分场出口浓度、二次电流和极板面积，无法独立回归四场效率。为避免手工指定，在四场未观测几何与迁移系数等效的最小信息假设下，令参考点各场贡献与 $U_{i,0}^2$ 成正比：
\begin{equation}
...
由删失运行点 $\hat\mu_c$ 和参考入口浓度 $C_{\mathrm{in},0}=37.41\gNm$ 计算出结合本设备后的基准工况等效去除指数
\begin{equation}
D_0=\log\left(\frac{1000C_{\mathrm{in},0}R_0}{\hat\mu_c}\right)=6.6846,
\label{eq:dref}
\end{equation}
其中 $R_0$ 为参考振打增益。
选择D-A穿透骨架，是基于物理逻辑、数据特征与工程约束三重契合的必然结果。

\begin{enumerate}
  \item \textbf{物理上，}D-A定律的指数穿透形式
  \[
  C_{\text{out}}=C_{\text{in}}e^{-D}
  \]
  精准对应题目描述的电晕放电、粉尘荷电、定向迁移与沉积分离这一完整物理链条，将”静电力驱动尘粒分离”的机理直接转化为电压平方项 \(U^{2}\) 对去除指数的贡献，比纯数据驱动的黑箱模型具有天然的因果解释力。

  \item \textbf{数据上，}面对$54.47\%$的出口浓度读数封顶于$50\mgNm$、有效信息严重压缩的现实困境，D-A定律无需数据覆盖全部工况区间，即可通过物理公式外推低浓度趋势，避开了回归模型在未观测特征空间中胡乱猜测的致命缺陷。

  \item \textbf{工程上，}四电场串联结构可自然分解为
  \[
  D=\sum w_{i}D_{i},
  \]
  各场的分场权重 \(w_{i}\) 赋予清晰的工程含义。
\end{enumerate}
\subsubsection{温度、积灰与振打修正}
高温会改变烟气密度、离子迁移及粉尘比电阻，实验研究表明温度影响具有明显非线性\cite{xu2015hightemp}。电除尘领域公认的温度-效率经验形状是钟形曲线。附件温度均值为
\begin{equation}
...
\end{align}
式\eqref{eq:dustindex}和\eqref{eq:platefactor}用稳态统计指标代替了瞬时追踪——式\eqref{eq:dustindex}定义了一个尘负荷指数$I_M$，将其是否超过阈值$1.45$作为判断周期过长的安全红线，而式\eqref{eq:platefactor}则用一个随周期延长而减小的折减因子$g_M$来描述积灰对除尘效率的持续损耗，二者协同作用，从约束和目标两个方向杜绝了无限延长振打周期来省电的可能性。

为避免D-A物理骨架将振打周期单调地视为"越短越好"（省电且清灰干净），从而错误地诱导优化器无限制缩短周期，我们引入振打再飞扬因子。振打使一部分尘饼落入灰斗，同时会造成短时再飞扬\cite{bush1984rapping,neundorfer1981rapping}。其相位积分后的浓度放大因子为
\begin{equation}
...
\subsection{时间正则化典型工况划基于GMM与Viterbi的典型工况划分}
...
\end{equation}
再用各维中位数与四分位距进行稳健标准化，降低离群值对工况划分的干扰以降低高温异常对尺度的影响。候选状态数 $K=3,\ldots,8$，每个候选拟合全协方差高斯混合模型
\begin{equation}
...
\label{eq:viterbi}
\end{equation}
驻留不足 15 分钟的短段合并至相邻低代价状态。有效模型需满足每个状态占比至少 1\%、中位驻留至少 15 分钟；在此基础上选择 贝叶斯信息准则BIC 最小者，若 BIC 差不超过 10，则取状态数较小者。Viterbi算法将基于数据相似性的GMM聚类结果赋予“时间连续性”，从而避免控制器因分钟级标签抖动而频繁动作，使后续生成的工况划分结果可以直接用于现场慢层调度。

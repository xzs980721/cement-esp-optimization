...
\begin{tikzpicture}[
  ...
  result/.style={rounded corners=1.4mm,draw=espteal,fill=green!5,minimum height=10mm,text width=3.05cm,align=center,font=\small},
...
\begin{keybox}
\begin{enumerate}
  \item \textbf{先诊断可识别性，再建模。} 将 $50\mgNm$ 量程封顶作为右删失纳入似然，明确拒绝对出口浓度作 $1/5$ 换算，也不以普通 $R^2$ 评价删失模型。
...
\begin{enumerate}
  ...
  \item $C_{\mathrm{out}}=50\mgNm$ 表示仪表上限处的右删失事件，即真实值不低于 50，而非真实值恰等于 50；50 个出口缺失值不插补；
